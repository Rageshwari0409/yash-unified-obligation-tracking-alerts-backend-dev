...
class LLMUsageTracker:
    ...
    def __init__(self, request: Request, model_name: str):
        ...
        self.auth_token = request.headers.get('Authorization', "")
        
        # --- Constants for the Kafka payload, sourced from environment variables ---
        self.agent_name = os.getenv("AGENT_NAME_CONSTANT", "OBLIGATION_TRACKING_ALERTS_AGENT")
        self.server_name = os.getenv("SERVER_NAME", "OBLIGATION_TRACKING_ALERTS_BACKEND")
        self.model_name_env = model_name

    def track_response(self, response: Any) -> Dict[str, Any]:
        """
    ...
    def _prepare_kafka_payload(self, record: Dict[str, Any], model_name: str) -> Dict[str, Any]:
        """
        ...
        thoughts_tokens = usage_meta.get("thoughtsTokenCount", 0) or (total_tokens - (prompt_tokens + completion_tokens))
        
        # Extract pure model name if it contains provider prefix
